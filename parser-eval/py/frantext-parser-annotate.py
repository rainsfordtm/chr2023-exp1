...
def script(annotator, lemma='', transitive=True):
    ...
    def am_i_locative(parent, children):
        ...
        if c0_lemma:
            ...
            if next_toks and not next_toks[0] == parent:
                ...
                if c1_lemma and \
                (next_toks[0].tags['conll_DEPREL'] == 'case' or \
                c1_lemma != 'de' or \
                (c1_lemma == 'de' and c0_lemma in ['fors'])):
                    c0_lemma += '+' + c1_lemma
    ...
    loc_transitive = []
    for arg in args:
        # We're not very interested in LABELS (LAS irrelevant)
        # But we do need the head of the dependent not to be a verb, 
        # and not a conjunction, interjection, or punctuation.
        # And we want to ignore things that the parser thinks are
        # subjects or objects.
        if arg.tags['conll_DEPREL'] in OBJECTS + SUBJECTS: continue
        if arg.tags['conll_CPOSTAG'] in ['AUX', 'CCONJ', 'INTJ', 'PUNCT', 'VERB']: continue
        # Also exclude 'en' pronoun
        if arg.tags['conll_CPOSTAG'] == 'PRON' and str(arg).lower() == 'en': continue
    ...
    for arg in args:
        ...
        if arg.tags['conll_DEPREL'] == 'expl:comp' and \
        re.match(r'-?[mts]|nou|vou', str(arg).lower()):
            reflexive_tok = arg
        # Second, check lone non-subject pronominals
        elif arg.tags['conll_CPOSTAG'] == 'PRON' and \
        not annotator.get_children(arg) and \
        not arg.tags['conll_DEPREL'] in SUBJECTS:
            # "se" is always reflexive
            if re.fullmatch(r"-?s('|e|oi)", str(arg).lower()):
                reflexive_tok = arg
            # Check non-subject / subject pairs
            for regex, subj in [
                (r"-?m('|e|oi)", 'je'),
                (r"-?t('|e|oi)", 'tu'),
                (r"-?nous", 'nous'),
                ...
                if re.fullmatch(regex, str(arg).lower()) and \
                subject_lemma == subj:
                    reflexive_tok = arg
    ...
    en_tok = None
    for arg in args:
        if arg.tags['conll_CPOSTAG'] == 'PRON' and \
    ...
    objects = []
    for arg in args:
        # Use object tag, filtering out arguments already recognized
        # as reflexives or clitic forms of en.
        # Also filter out c', can't be DO, yes I found it, no I don't
        # know why.
        # Apply the "object_allowed" function too, which is far more
        # particular about annotating DOs for verbs which should in
        # principle be intransitive.
        if arg.tags['conll_DEPREL'] == 'obj' and \
        not arg is en_tok and not arg is reflexive_tok and \
        not str(arg).lower() == "c'" and \
        object_allowed(args):
            objects.append(annotator.get_string(arg))
        # Single "le" and "les" are always direct objects, occasionally
        # mistagged.
        elif re.fullmatch(r"-?l('|e|a|es)", str(arg).lower()) and \
        not annotator.get_children(arg):
            objects.append(annotator.get_string(arg))
